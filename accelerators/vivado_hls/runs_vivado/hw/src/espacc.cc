...
void compute(word_t _inbuff[SIZE_IN_CHUNK_DATA],
             /* <<--compute-params-->> */
	 const unsigned data_out_size,
	 const unsigned data_in_size, int chunk,  f_word_t _tmpbuff[64],
            f_word_t _outbuff[SIZE_OUT_CHUNK_DATA])
{
    ...
    const unsigned out_length = round_up(data_out_size, VALUES_PER_WORD) / 1;
     ...
     for (int i = 0; i < length; i++){
        _tmpbuff[0] = _tmpbuff[0] + _inbuff[i]; //S
     }

    #ifndef SYNTHESIS
    _tmpbuff[1] =  _tmpbuff[0].to_float()/(float)data_in_size; //PI
        ...
        for (int i = 0; i < length; i++){
            if(i == 0){
                 #ifndef __SYNTHESIS__
                    std::cout << "In chunk pass " << std::endl;
                #endif 
                if(chunk != 0){
                     
                   if(_tmpbuff[3] != _inbuff[i]){
                ...
                if(_inbuff[i - 1] != _inbuff[i]){
                        _tmpbuff[5] = _tmpbuff[5] + 1;
                   }
            }
        

        if(fabs(_tmpbuff[1].to_float() - 0.5) > (2.0 / sqrt(data_in_size))){
        _tmpbuff[2] = 0; 
    }
    else{
    ...
    #ifndef __SYNTHESIS__
        std::cout << "V " << _tmpbuff[5] << std::endl;
    #endif 

    #else
      _tmpbuff[1] =  _tmpbuff[0]()/(float)data_in_size; //PI
        ...
        if(fabs(_tmpbuff[1]() - 0.5) > (2.0 / sqrt(data_in_size))){
        _tmpbuff[2] = 0; 
    }
    else{
    ...
    _tmpbuff[3] = _inbuff[4095]; //for next chumk, preserve last element
    
    for (int i = 0; i < out_length; i++)
    {
        _outbuff[i] = _tmpbuff[2];
        }
