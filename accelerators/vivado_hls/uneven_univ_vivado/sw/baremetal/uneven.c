...
const int32_t data_stop = 8100;
const int32_t data_L = 2;
...
/* <<--regs-->> */
#define UNEVEN_DATA_STOP_REG 0x4c
#define UNEVEN_DATA_L_REG 0x48
...
static void init_buf (word_t *in, word_t * gold)
{
	...
	for (i = 0; i < 1; i++)
		for (j = 0; j < data_in_size; j++)
			in[i * in_words_adj + j] = (word_t) (j/5)%2;
...
int main(int argc, char * argv[])
{
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
		iowrite32(dev, UNEVEN_DATA_STOP_REG, data_stop);
		iowrite32(dev, UNEVEN_DATA_L_REG, data_L);
